...
const transformActivitiesData = (activities) => {
  ...
  return activities.map(activity => {
    const {icon, type} = getActivityIconAndType(activity.activity_type)
    const displayText = generateActivityDisplayText(activity)
...
const generateActivityDisplayText = (activity) => {
  const {user_name, activity_type, description, resource_name, activity_type_display} = activity
  const actionText = activity_type_display || getActivityActionText(activity_type)

  if (resource_name) {
    return `${user_name}${actionText}“${resource_name}”`
  }

  return description
}

const getActivityActionText = (activityType) => {
  const actionMap = {
    'data_source_created': '创建了数据源',
    'dataset_created': '创建了数据集',
    'pipeline_executed': '执行了处理流程',
    'visualization_created': '创建了可视化',
    'dashboard_created': '创建了看板',
    'ai_model_trained': '训练了AI模型',
    'data_processed': '处理了数据'
  }

  return actionMap[activityType] || '执行了操作'
}
