...
class VisualizationViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.request.method in ['GET', 'data']:
            # 查看操作对所有认证用户开放
    ...
    def perform_create(self, serializer):
        visualization = serializer.save(created_by=self.request.user)
        # 创建可视化活动记录 - 修复调用
        from activities.utils import create_visualization_activity
        result = create_visualization_activity(
        ...
        if result:
            print(f"✅ 可视化活动记录创建成功: {result.description}")
        else:
            print(f"❌ 可视化活动记录创建失败")
    ...
    def perform_destroy(self, instance):
        ...
        if instance.created_by != self.request.user and self.request.user.role != 'admin':
            raise permissions.PermissionDenied("您只能删除自己创建的可视化")
...
class DashboardViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        print("🎯 [DashboardViewSet] perform_create 被调用")
        print(f"🎯 请求用户: {self.request.user.username}")
        ...
        # 创建活动记录 - 使用正确的活动类型
        self._create_dashboard_activity(dashboard, 'created')

    def perform_update(self, serializer):
        dashboard = serializer.save()
        print(f"🎯 [DashboardViewSet] perform_update 被调用: {dashboard.name}")
        self._create_dashboard_activity(dashboard, 'updated')

    def perform_destroy(self, instance):
        print(f"🎯 [DashboardViewSet] perform_destroy 被调用: {instance.name}")
        self._create_dashboard_activity(instance, 'deleted')
        super().perform_destroy(instance)
    ...
    def _create_dashboard_activity(self, dashboard, action):
        """创建看板活动记录的辅助方法"""
        try:
            ...
            if result:
                print(f"✅ 看板活动记录创建成功: {result.description}")
            else:
                print(f"❌ 看板活动记录创建失败")
                # 备用方案：直接创建
                self._create_activity_directly(dashboard, action)

        except Exception as e:
            print(f"❌ 创建看板活动记录异常: {str(e)}")
            # 备用方案：直接创建
            self._create_activity_directly(dashboard, action)

    def _create_activity_directly(self, dashboard, action):
        """直接创建活动记录的备用方法"""
        try:
            ...
            # 使用正确的活动类型映射
            action_map = {
                'created': ('创建了看板', ActivityType.DASHBOARD_CREATED),
                'updated': ('更新了看板', ActivityType.DASHBOARD_UPDATED),
                'deleted': ('删除了看板', ActivityType.DASHBOARD_DELETED),
            }

            if action in action_map:
                action_text, activity_type = action_map[action]
                description = f'{self.request.user.username} {action_text} "{dashboard.name}"'
                ...
                activity = Activity.objects.create(
                    ...
                    metadata={'source': 'direct_backup'}
                )
                print(f"✅ 备用方法创建活动记录成功: {description} (ID: {activity.id})")

        except Exception as e:
            print(f"❌ 备用方法创建活动记录也失败: {str(e)}")


@api_view(['POST'])
@permission_classes([permissions.IsAuthenticated])
def debug_dashboard_create(request):
    """
    调试端点：检查看板创建过程
    """
    print("🎯 [debug_dashboard_create] 调试端点被调用")
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            # 创建活动记录
            from activities.utils import create_dashboard_activity
            activity_result = create_dashboard_activity(
                user=request.user,
                dashboard_name=dashboard.name,
                dashboard_id=dashboard.id,
                action='created'
            ...
            return Response({
                ...
                'dashboard_id': dashboard.id,
                'dashboard_name': dashboard.name,
                'activity_created': bool(activity_result),
                'activity_id': activity_result.id if activity_result else None
            })
        else:
            return Response({
                'success': False,
