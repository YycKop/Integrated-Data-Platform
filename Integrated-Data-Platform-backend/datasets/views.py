...
class DataSourceViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        try:
            ...
            if result:
                print(f"✅ [DataSource] 成功创建数据源活动记录")
            else:
...
class DatasetViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related('data_source')
...
class DataRecordViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = DataRecord.objects.filter(dataset__created_by=self.request.user)
        ...
        if search_keyword:
            # 在JSON字段中搜索
            queryset = queryset.filter(data__icontains=search_keyword)
